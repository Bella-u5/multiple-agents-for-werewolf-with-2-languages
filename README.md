...
> **🚀 在线体验：https://bella-u5.github.io/multiple-agents-for-werewolf-with-2-languages/**
...
React 19 + TypeScript + Vite (前端 / 游戏状态机)
    ...
    │     └── ...                 其他角色 Agent
    └── services/agents.ts      Gemini 2.5 Flash 调用层（策略化 Agent 基类 + 各角色实现）
...
打开 http://localhost:3000。**API Key（BYOK）**：首次进入游戏设置页，在「LLM 引擎」面板粘贴你的 Gemini API Key（仅保存在浏览器 localStorage，不上传服务器）；不填也可以「离线演示模式」完整跑完整局。本地开发也可选在 `.env.local` 设置 `GEMINI_API_KEY` 供构建注入。
